		  ...
		  while True:
		    try:
		      ...
		      return n
			    ...
			    print ("Debe ingresar un número, no otro tipo de dato.")
			...
			while True:
			  opc = askStr("\nIngrese una opción del menú:")
				if opc in ["+", "-", "*", "/", "5"]:
					...
					print("La opción no está contenida en las operaciones disponibles.")
...
```python
#No encontré excepciones necesarias para incluir.
		def ask_str(msj):
			m=str(input(msj))
			return m
...
```python
#Creación del único error que puede resultar:
